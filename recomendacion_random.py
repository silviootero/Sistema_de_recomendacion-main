...
@st.cache
def load_modelD():
    with lzma.open('lista_de_juegos2.pkl', 'rb') as file:
        B = pickle.load(file)
        return B
...
@st.cache
def load_modelE():
    with lzma.open('header2.pkl', 'rb') as file:
        A = pickle.load(file)
        return A

lista_videojuegos = load_modelE()

def main():
    df = pd.read_csv('base_original_random.csv', encoding="utf-8")
    df2 = pd.read_csv('base_actualizada_random.csv', encoding="utf-8")
    ...
    def actualizar_csv(data1, data2, header, nueva_info):
        ...
        if a == b:
            a.append(nueva_info)
            with open('base_actualizada_random.csv', 'w', encoding="utf-8") as file:
